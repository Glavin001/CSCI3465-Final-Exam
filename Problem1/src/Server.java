				...
				try {
					...
					try {
						...
						if (p.getToken().equals(Token.CONNECT)) {
							...
							// Process Loop
							while (!socket.isClosed() && !socket.isOutputShutdown() && !socket.isInputShutdown() ) {
								
								// Tell Client that Server is ready.
								this.output.writeObject(new Protocol(Token.READY,
										null));
								...
								case DIVIDE: {
									...
									this.output.writeObject(new Protocol(
											Token.CONST2, String.valueOf(result)));
