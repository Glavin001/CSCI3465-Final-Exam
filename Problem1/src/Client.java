			...
			try {
				...
				while (isRunning && !this.socket.isClosed()) {
					...
					if (p.getToken().equals(Token.READY)) {
						...
						// Prompt for input
						System.out.print("Enter command (PLUS, MINUS, MULTIPLY, DIVIDE, QUIT): ");
						...
						if (isRunning) {
							...
							this.output.writeObject(new Protocol(Token.CONST1, c1));
							this.output.writeObject(new Protocol(Token.CONST1, c2));
							
							// Receive result CONST2
							Protocol result = (Protocol) this.input.readObject();
							
							// Print result
							System.out.println("Result: "+result.getArgument());
